...
const Sidebar = ({ expand, setExpand }) => {
  ...
  const {openMenu, setOpenMenu}=useAppContext({id:0, open:false});
  return (
    <div
      className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 
        ...
        ${expand ? 'p-4 w-64' : 'md:w-20 w-[72px]'}`}
    >
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
          ...
          >
            ...
            <div
              className={`absolute w-max opacity-0 group-hover:opacity-100 transition 
                bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none
                ${expand ? "left-1/2 -translate-x-1/2 top-12" : "left-12 top-1/2 -translate-y-1/2"}`}
            >
...
>
            ...
            <div className={`absolute -top-60 pb-8 ${
                !expand && "-right-40"}opacity-0 hover:opacity-100 hidden group-hover:block transition`}>
